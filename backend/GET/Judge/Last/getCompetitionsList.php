...
if(isset($postdata) && !empty($postdata)){
    ...
    $sql = "SELECT DISTINCT(adj_sign_name), name, surname, comp_event_name, last_dance_style_id, last_ds_name, last_dance_cat_id, last_dc_name, last_age_group_id, last_age_name, heat_id, heat_name, component_id, component_name, component_shortname, flag_dance_category FROM competition 
    ...
    INNER JOIN users ON users.user_id=adjucator_round.competition_adjucator_id WHERE users.username = '{$user}'";
    ...
    if($result = mysqli_query($con,$sql)) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            ...
            $list[$i]['flag_dc'] = $row['flag_dance_category'];
            $list[$i]['dance_style_id'] = $row['last_dance_style_id'];
            $list[$i]['dance_category_id'] = $row['last_dance_cat_id'];
            $list[$i]['age_group_id'] = $row['last_age_group_id'];
            $i++;
        }
